...
Identity.prototype.authorize = function (options, callback) {
  ...
  request(authenticationOptions, function (err, response, body) {
    ...
    // If we get a bad response, return an error.
    else if (!body || !body.access || !body.access.token) {
        callback(new Error('SDS-FIX: Bad response from authentication provider: ' + body));
    }
